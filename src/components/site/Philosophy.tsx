...
export function Philosophy() {
  return (
    <section id="filosofia" className="py-24 md:py-40 bg-paper">
      <div className="mx-auto max-w-[1400px] px-6 md:px-12">
        <div className="reveal text-center max-w-4xl mx-auto mb-20">
          <span className="eyebrow">— Sala 03 / Manifiesto</span>
          <p className="mt-10 font-display text-3xl md:text-5xl lg:text-6xl leading-[1.1] text-foreground">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-border mt-24">
          {pillars.map((p, i) => (
            <div
              key={p.k}
              className="reveal bg-paper p-10 md:p-14 hover:bg-pearl transition-colors duration-500"
              style={{ transitionDelay: `${i * 40}ms` }}
            >
              <span className="eyebrow">0{i + 1}</span>
              <h3 className="font-display text-3xl md:text-4xl mt-5 mb-4">{p.k}</h3>
